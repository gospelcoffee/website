...
> **v0.8.3 change:** The footer gains a fourth item, a **Jobs** link (`https://tinyurl.com/teamvalo`, opens in a new tab) sitting third, between the email address and the copyright. It is a plain `.footer-simple` anchor and needs no new CSS. Clicks are tracked in GA4 as the event **`jobs_open`** (`ux_area: footer`, `ux_action: open_jobs`, `ux_detail: footer_link`), registered in the `META` map in `assets/js/analytics.js` and fired by the existing delegated `[data-ga]` click listener — no new JavaScript. It is deliberately **not** a `ROLLUP_EVENTS` entry, since the three rollups (`view_menu` / `view_location` / `get_directions`) all represent visit intent and a jobs click is a different funnel. The footer still carries no location cards, addresses, hours, or "Prescott, Arizona"; `404.html` and the redirect stubs are unchanged.
...
- **Required lexicon (v0.8.1).** Use these exact terms in all visitor copy and JSON-LD:
  ...
  - **"variety" / "varieties"** — never "varietal" / "varietals" (when referring to coffee/bean kinds).
...
- **Modern espresso only: never name a non-espresso brew method.** Every coffee Valo serves is espresso based. "pour over" is retired sitewide and must not be reintroduced, and it must not be replaced with the new method either. No "americano", "long black", "filter", "drip", or "batch brew" in visitor copy or structured data. Espresso-based descriptions remain correct and are unaffected ("Espresso with steamed milk", "Espresso cut with steamed milk", "A single or double shot of espresso"). The coffee tasting flight is described as **"a tasting flight"** and never by how it is prepared. Green-coffee processing terms (washed, natural, wet-hulled, Swiss Water) describe work done at origin, not brewing, and are not covered by this rule.
...
Coffee                 Bean Menu                          Optional
   latte         7        dark & smoky    peru               vanilla
...
Each entry in `beans.items` in `data/menu.json` may carry optional sourcing keys alongside `name` and `origin`: `producer`, `farm`, `region`, `variety`, `process`, `certification`, `elevation`, `lot`, `harvest`, `screen`, `grade`. Omit any key the supplier's lot sheet does not support rather than writing an empty string or guessing.

Rules:
- **Nothing here ever renders.** `menu.js` reads only `name` and `origin`, so these keys are inert on the page. Do not add them to the visible menu, the homepage preview, or the no-JS fallbacks. The Bean Menu row stays name + origin.
- **The only public surface is the JSON-LD `description`** on the matching `MenuItem` in both the Bean Menu and Coffee beans `MenuSection`s, phrased as a sentence: origin, then place and producer, then varieties and process. This is read by crawlers and never displayed to visitors.
- **Internal-only keys** (`lot`, `harvest`, `screen`, `grade`, `elevation`) stay out of the JSON-LD. A supplier reference like `P614006-2` or a screen size adds noise without adding a searchable term.
- **Never over-claim certification.** Only state Fairtrade, Organic or similar when the lot sheet does. A lot graded "Conventional" carries no certification line at all.
- Use **"variety" / "varieties"**, never "varietal" (section 3 lexicon). Regional cooperative lots have no named grower and no `farm` — say so plainly rather than inventing a farm name.
- Values must trace to a primary source (the supplier's lot page). Anything inferred rather than lot-confirmed is flagged in `TODO.md` until verified.
...
- **Tasting experience** opens with the availability note at the top: "Tasting experiences are available at Valo Lab only." Then coffee tasting experience (19) and not coffee tasting experience (19), each with a one-line description using identical sentence structure. The two descriptions are: "Four coffees of your choice, served as a tasting flight, then any full-size drink from the menu." and "Four not coffee lattes of your choice, served as a sample flight, then any full-size drink from the menu." These must stay identical across `data/menu.json`, the JSON-LD MenuItems, the static no-JS fallback and `COPY_DECK.md`. Frame as a feature, not a disclaimer; never use "inside the Prescott Resort only" as the primary wording.
...
19. Footer contains only phone, email, the Jobs link, and copyright; no duplicate location block. The Jobs link opens `https://tinyurl.com/teamvalo` in a new tab and fires the GA4 `jobs_open` event.
