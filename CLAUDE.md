...
> **v0.8.1 change:** The philosophy section is now **"Our Philosophy"** (eyebrow + comment). A required **lexicon** is enforced sitewide: "flavor notes" (never "tasting notes"/"flavor profiles"), "naturally occurring flavor notes" (never "naturally occurring flavors"), "varietal/varietals" (never "variety/varieties"); the earlier approved-brief verbatim lock is retired. The collapsible "Where do the tasting notes come from?" is now "Where do the flavor notes come from?", and a **fifth** collapsible, **"Where does our coffee come from?"**, was added after it as an **intentional empty placeholder** (no body copy yet) — it is **excluded from the FAQ JSON-LD** until the owner supplies copy, so the FAQPage maps the **four populated** questions 1:1. In the espresso answer, "smoothness" → "balance". Both location descriptions are unified to one canonical wording sourced from `data/locations.json` (`loc.blurb`), rendered identically in the Visit Valo cards and the Visit modal. Still **deferred pending owner copy:** off-gassing relocation (item 5), the modern-espresso rebuild (item 6), and tightening "Why is the menu so simple?" (item 8). The accordion/`<details>` format is **kept**.
...
5. **Our Philosophy** — anchor `#philosophy`, the last content section. Eyebrow **"Our Philosophy"**, then the opening lede ("Every drink on our menu is made with pure, raw ingredients. We do not add anything else, so the bean's naturally occurring flavor notes can be enjoyed clearly."), then **five** collapsible `<details class="philosophy-detail">` questions, in order: **What is coffee?** → **Where do the flavor notes come from?** → **Where does our coffee come from?** → **Why is the menu so simple?** → **How do we make espresso?** All start collapsed and use the same small-caps summary styling with a `+` that flips to `−`. **"Where does our coffee come from?" is an intentional empty placeholder** (summary only, no body) pending owner-supplied copy — keep it until that copy lands. There is no separate philosophy page and no "Read our philosophy" link. (Further rebuilds — off-gassing relocation, modern-espresso rewrite, tightening "Why is the menu so simple?" — remain **deferred**; see the v0.8.1 note.)
...
- `status.js` exposes `window.ValoStatus.getStatus(location)` → `{ isOpen, primary, secondary, closesInMinutes, closesSoon }`, reused by the bottom smart CTA and the Visit modal. `closesInMinutes > 60` is the "stays open more than an hour" test for the CTA label.
...
- **FAQPage** (`@id https://valocoffee.com/#philosophy`) with the **four populated** philosophy questions/answers, mapping 1:1 to the four populated on-page collapsibles. The fifth collapsible ("Where does our coffee come from?") is an empty placeholder and is **intentionally excluded** from the FAQPage until it has body copy (an empty Answer would be an invalid rich result). Re-add it to the @graph the moment its copy lands so 1:1 is restored.
...
16. Philosophy lives on the homepage as the "Our Philosophy" section with five collapsible questions (the fifth, "Where does our coffee come from?", an intentional empty placeholder pending copy and excluded from the FAQ JSON-LD); "How do we make the perfect drink?" is absent. Lexicon enforced: "flavor notes" / "naturally occurring flavor notes" / "varietal(s)" — no "tasting notes", "flavor profiles", "naturally occurring flavors", "variety/varieties", or "smoothness". (Items 5/6/8 deferred per the v0.8.1 note.)
