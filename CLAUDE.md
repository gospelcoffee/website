...
1. **Header** — logo, simple nav (Locations, Menu, Philosophy). No pills, no oversized header. A previous "Get directions" nav item was removed because it duplicated Locations. Directions are still reachable via the Get directions buttons inside each location card and footer. **Dual-mode behavior:** on the home page, every nav item is an in-page anchor (`#today`, `#menu`, `#philosophy`) so the visitor never leaves the home page from the header. On every other page (locations.html, menu.html, philosophy.html), the same nav items link to the corresponding .html files like normal. The rationale is that the home page is the visitor's hub: the header moves them around the page, and the deeper pages are reached via the section CTAs (View full menu, Get directions on each location card, Read our philosophy). This keeps the home page journey simple.
